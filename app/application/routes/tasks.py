...
from app.application.models import Task
from app.application.utils import token_required
from cffi.backend_ctypes import unicode
import bleach

bp = Blueprint('tasks', __name__, url_prefix='/todo/api/v1.0/tasks')

def make_public_task(task):
    new_task = {}
    for field in task.__dict__:
        if field == 'id':
            new_task['uri'] = url_for('tasks.get_task', task_id=task.id, _external=True)
